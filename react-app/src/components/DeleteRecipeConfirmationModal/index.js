...
function DeleteRecipeConfirmationModal({ recipe }) {
    ...
    const confirmDelete = async () => {
        ...
        const data = await dispatch(deleteRecipeThunk(recipe.id))
        if (data) {
    ...
    if (recipe.author.id !== sessionUser.id) closeModal();

    return (
        <div className="delete_confirmation_modal_div">
            <h2 className="delete_confirmation_modal_title">Delete {recipe.title}</h2>
            <p className="delete_confirmation_modal_info">Are you sure you want to delete {recipe.title}?</p>
...
export default DeleteRecipeConfirmationModal;
